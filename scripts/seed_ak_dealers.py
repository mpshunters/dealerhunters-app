"""
Seed Alaska motor vehicle dealers via Google Places API city sweep.
AK is dominated by Anchorage (40% of state population). Fairbanks is
the Interior hub; Juneau is the capital but road-inaccessible.
Wasilla and Palmer serve the Mat-Su Valley north of Anchorage.
Sweeps ~18 cities; deduplicates by google_place_id; only keeps state=AK.
"""
...
print("DealerHunters Alaska city-sweep seeder starting...")
...
AK_CITIES = [
    ...
    # Matanuska-Susitna Valley (fastest-growing region)
    "Wasilla", "Palmer", "Willow", "Big Lake",
    # Interior
    "Fairbanks", "North Pole",
    # Kenai Peninsula
    "Kenai", "Soldotna", "Homer", "Seward",
    # Southeast (panhandle — limited road access, but dealers exist)
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} Alaska", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "AK").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing AK dealerships in DB\n")
print(f"Sweeping {len(CITIES)} Alaska cities...\n")
...
print(f"\nAlaska sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "AK").execute()
print(f"  Total AK dealerships in DB: {result.count}")
