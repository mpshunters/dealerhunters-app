"""
Seed Arkansas motor vehicle dealers via Google Places API city sweep.
AR is anchored by Little Rock metro and northwest Arkansas (Fayetteville /
Bentonville / Rogers / Springdale — one of the fastest-growing metros in the US).
Sweeps ~55 cities; deduplicates by google_place_id; only keeps state=AR.
"""
...
print("DealerHunters Arkansas city-sweep seeder starting...")
...
AR_CITIES = [
    ...
    "Bryant", "Maumelle", "Sherwood", "Jacksonville", "Cabot",
    "Lonoke", "Searcy", "Beebe",
    # Northwest Arkansas (Fayetteville–Bentonville corridor)
    "Fayetteville", "Springdale", "Rogers", "Bentonville",
    "Bella Vista", "Siloam Springs", "Centerton", "Lowell",
    "Cave Springs", "Highfill",
    # Fort Smith / River Valley
    "Fort Smith", "Van Buren", "Greenwood", "Alma",
    "Clarksville", "Russellville", "Morrilton",
    # Jonesboro / Northeast AR
    "Jonesboro", "Paragould", "Blytheville", "Osceola",
    "West Memphis", "Marion", "Forrest City",
    # Pine Bluff / Southeast AR
    "Pine Bluff", "White Hall", "Arkadelphia", "Hot Springs",
    "Hot Springs Village", "Malvern", "Benton",
    # Texarkana area
    "Texarkana", "Hope", "El Dorado", "Magnolia",
    "Camden", "Crossett",
    # Other
    "Mountain Home", "Harrison", "Batesville", "Heber Springs",
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} Arkansas", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "AR").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing AR dealerships in DB\n")
print(f"Sweeping {len(CITIES)} Arkansas cities...\n")
...
print(f"\nArkansas sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "AR").execute()
print(f"  Total AR dealerships in DB: {result.count}")
