"""
Seed Arizona motor vehicle dealers via Google Places API city sweep.
AZ is dominated by the Phoenix Valley (Maricopa County) which contains
~80% of the state's population. Tucson is the #2 market.
Sweeps ~70 cities; deduplicates by google_place_id; only keeps state=AZ.
"""
...
print("DealerHunters Arizona city-sweep seeder starting...")
...
AZ_CITIES = [
    # Phoenix — core
    "Phoenix", "Mesa", "Chandler", "Glendale", "Scottsdale",
    "Gilbert", "Tempe", "Peoria", "Surprise", "Goodyear",
    "Avondale", "Buckeye", "Litchfield Park", "Tolleson",
    "El Mirage", "Youngtown", "Sun City", "Sun City West",
    "Anthem", "New River", "Cave Creek", "Carefree",
    # East Valley
    "Queen Creek", "San Tan Valley", "Maricopa",
    "Apache Junction", "Gold Canyon",
    # Northeast Valley
    "Paradise Valley", "Fountain Hills", "Scottsdale",
    "Rio Verde", "Carefree",
    # West Valley
    "Laveen", "Tolleson", "Avondale", "Goodyear",
    "Buckeye", "Wickenburg",
    # Phoenix metro — Pinal County
    "Casa Grande", "Coolidge", "Eloy", "Florence",
    # Tucson metro
    "Tucson", "Marana", "Oro Valley", "Sahuarita",
    "Green Valley", "Vail", "Rincon Valley",
    "South Tucson", "Drexel Heights",
    # Northern AZ
    "Flagstaff", "Sedona", "Prescott", "Prescott Valley",
    "Chino Valley", "Cottonwood", "Camp Verde", "Williams",
    "Winslow", "Show Low", "Pinetop-Lakeside",
    # Western AZ
    "Yuma", "San Luis", "Somerton", "Wellton",
    "Lake Havasu City", "Bullhead City", "Kingman",
    "Fort Mohave", "Golden Valley",
    # Southern AZ
    "Sierra Vista", "Bisbee", "Douglas", "Nogales",
    "Safford", "Thatcher",
]
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} Arizona", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "AZ").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing AZ dealerships in DB\n")
print(f"Sweeping {len(CITIES)} Arizona cities...\n")
...
print(f"\nArizona sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "AZ").execute()
print(f"  Total AZ dealerships in DB: {result.count}")
