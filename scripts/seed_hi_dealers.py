"""
Seed Hawaii motor vehicle dealers via Google Places API city sweep.
HI is concentrated on Oahu (Honolulu metro ~1M). Maui, Big Island,
and Kauai each have clusters of dealers in their main towns.
Sweeps ~22 cities/communities; deduplicates by google_place_id; only keeps state=HI.
"""
...
print("DealerHunters Hawaii city-sweep seeder starting...")
...
HI_CITIES = [
    # Oahu — Honolulu metro
    "Honolulu", "Pearl City", "Aiea", "Waipahu", "Kapolei",
    "Ewa Beach", "Mililani", "Kaneohe", "Kailua", "Kāneʻohe",
    # Maui
    "Kahului", "Wailuku", "Kihei", "Lahaina", "Makawao",
    # Big Island (Hawaii)
    "Hilo", "Kailua-Kona", "Waimea", "Pahoa",
    # Kauai
    "Lihue", "Kapaa",
    # Molokai / Lanai (small but included for completeness)
    "Kaunakakai",
]
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} Hawaii", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "HI").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing HI dealerships in DB\n")
print(f"Sweeping {len(CITIES)} Hawaii cities...\n")
...
print(f"\nHawaii sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "HI").execute()
print(f"  Total HI dealerships in DB: {result.count}")
