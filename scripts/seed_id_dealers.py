"""
Seed Idaho motor vehicle dealers via Google Places API city sweep.
ID is anchored by the Boise–Nampa–Meridian–Caldwell metro (Treasure Valley),
one of the fastest-growing metro areas in the US. Idaho Falls and Pocatello
serve the southeast; Coeur d'Alene the north.
Sweeps ~30 cities; deduplicates by google_place_id; only keeps state=ID.
"""
...
print("DealerHunters Idaho city-sweep seeder starting...")
...
ID_CITIES = [
    # Treasure Valley — Boise metro (fastest-growing in US)
    "Boise", "Meridian", "Nampa", "Caldwell", "Eagle",
    "Star", "Kuna", "Middleton", "Mountain Home",
    # Southeast Idaho
    "Idaho Falls", "Ammon", "Pocatello", "Chubbuck",
    "Blackfoot", "Rexburg", "Burley", "Twin Falls",
    "Jerome", "Rupert",
    # South-central
    "Hailey", "Ketchum",
    # Eastern / Bear River
    ...
    "Coeur d'Alene", "Post Falls", "Rathdrum", "Hayden",
    "Sandpoint", "Moscow", "Lewiston", "Clarkston",
]
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} Idaho", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "ID").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing ID dealerships in DB\n")
print(f"Sweeping {len(CITIES)} Idaho cities...\n")
...
print(f"\nIdaho sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "ID").execute()
print(f"  Total ID dealerships in DB: {result.count}")
