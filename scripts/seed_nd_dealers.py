"""
Seed North Dakota motor vehicle dealers via Google Places API city sweep.
ND is anchored by Fargo–West Fargo in the east, Bismarck–Mandan in the
center, and Minot in the north. Williston serves the Bakken oil patch.
Sweeps ~22 cities; deduplicates by google_place_id; only keeps state=ND.
"""
...
print("DealerHunters North Dakota city-sweep seeder starting...")
...
ND_CITIES = [
    # Fargo metro
    "Fargo", "West Fargo", "Moorhead",  # Moorhead is MN but dealers bleed over — filtered by state
    "Wahpeton", "Valley City", "Jamestown",
    # Bismarck–Mandan (capital)
    "Bismarck", "Mandan", "Lincoln",
    # Grand Forks metro
    "Grand Forks", "Grafton",
    # Minot metro
    "Minot", "Burlington", "Bottineau",
    # Bakken oil patch — west
    "Williston", "Watford City", "Dickinson", "Beulah",
    # Southeast / south-central
    "Devils Lake", "Lisbon", "Ellendale", "Oakes",
    # Northeast
    "Cavalier", "Rolla",
]
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} North Dakota", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "ND").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing ND dealerships in DB\n")
print(f"Sweeping {len(CITIES)} North Dakota cities...\n")
...
print(f"\nNorth Dakota sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "ND").execute()
print(f"  Total ND dealerships in DB: {result.count}")
