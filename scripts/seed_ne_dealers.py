"""
Seed Nebraska motor vehicle dealers via Google Places API city sweep.
Sweeps ~50 cities; deduplicates by google_place_id; only keeps state=NE.
"""
...
print("DealerHunters Nebraska city-sweep seeder starting...")
...
NE_CITIES = [
    ...
    "La Vista", "Ralston", "Gretna", "Elkhorn", "Millard",
    "Chalco", "Sarpy", "Plattsmouth", "Wahoo", "Ashland",
    # Lincoln metro
    "Waverly", "Seward", "York", "Aurora", "Geneva",
    "Crete", "Beatrice", "Fairbury", "Falls City",
    # Central Nebraska
    "Grand Island", "Kearney", "Hastings", "Lexington", "Holdrege",
    "McCook", "North Platte", "Ogallala", "Sidney", "Alliance",
    "Scottsbluff", "Gering", "Chadron", "Valentine",
    # Northeast Nebraska
    "Norfolk", "Wayne", "Colfax", "Madison", "Pierce",
    "O'Neill", "Ainsworth", "Neligh",
    # Southeast Nebraska
    "Columbus", "Schuyler", "David City", "Nebraska City",
    "Auburn", "Syracuse", "Tecumseh",
]
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} Nebraska", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "NE").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing NE dealerships in DB\n")
print(f"Sweeping {len(CITIES)} Nebraska cities...\n")
...
print(f"\nNebraska sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "NE").execute()
print(f"  Total NE dealerships in DB: {result.count}")
