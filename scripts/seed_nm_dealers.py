"""
Seed New Mexico motor vehicle dealers via Google Places API city sweep.
NM is anchored by Albuquerque (metro ~920k) and its suburb Rio Rancho.
Las Cruces serves the south; Santa Fe draws luxury buyers; Farmington
anchors the Four Corners energy market.
Sweeps ~28 cities; deduplicates by google_place_id; only keeps state=NM.
"""
...
print("DealerHunters New Mexico city-sweep seeder starting...")
...
NM_CITIES = [
    # Albuquerque metro (central)
    "Albuquerque", "Rio Rancho", "Bernalillo", "Los Lunas",
    "Belen", "Edgewood", "Moriarty",
    # Santa Fe / north-central
    "Santa Fe", "Espanola", "Los Alamos", "Taos",
    "Las Vegas",  # NM, not NV
    # Northwest / Four Corners
    "Farmington", "Aztec", "Bloomfield", "Gallup",
    # Southeast (oil — Permian Basin spills in)
    "Roswell", "Hobbs", "Carlsbad", "Artesia",
    "Lovington", "Portales", "Clovis",
    # South / Las Cruces metro
    "Las Cruces", "Sunland Park", "Deming", "Silver City",
    # South-central
    "Alamogordo", "Ruidoso",
    # Northeast
    "Raton", "Tucumcari",
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} New Mexico", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "NM").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing NM dealerships in DB\n")
print(f"Sweeping {len(CITIES)} New Mexico cities...\n")
...
print(f"\nNew Mexico sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "NM").execute()
print(f"  Total NM dealerships in DB: {result.count}")
