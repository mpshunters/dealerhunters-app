"""
Seed Utah motor vehicle dealers via Google Places API city sweep.
UT is dominated by the Wasatch Front (Salt Lake City / Provo / Ogden corridor),
which holds ~80% of the state's population. St. George is a fast-growing market.
Sweeps ~55 cities; deduplicates by google_place_id; only keeps state=UT.
"""
...
print("DealerHunters Utah city-sweep seeder starting...")
...
UT_CITIES = [
    ...
    "South Jordan", "Draper", "Riverton", "Herriman", "Bluffdale",
    "Murray", "Midvale", "Taylorsville", "Holladay", "Millcreek",
    "Cottonwood Heights", "South Salt Lake",
    # Davis County (north of SLC)
    "Layton", "Clearfield", "Syracuse", "Clinton", "Roy",
    "Kaysville", "Farmington", "Centerville", "Bountiful",
    "North Salt Lake", "Woods Cross",
    # Weber County / Ogden
    "Ogden", "Roy", "Riverdale", "Washington Terrace",
    "South Ogden", "Harrisville", "North Ogden", "Pleasant View",
    "Farr West", "Plain City", "Brigham City",
    # Utah County / Provo
    "Provo", "Orem", "Lehi", "American Fork", "Pleasant Grove",
    "Lindon", "Springville", "Spanish Fork", "Payson",
    "Santaquin", "Eagle Mountain", "Saratoga Springs",
    # St. George / Washington County
    "St. George", "Washington", "Hurricane", "Ivins",
    "Santa Clara", "Bloomington", "Cedar City",
    # Cache Valley
    "Logan", "North Logan", "Hyde Park", "Smithfield",
    # Eastern / rural UT
    "Price", "Moab", "Vernal", "Roosevelt",
]
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} Utah", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "UT").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing UT dealerships in DB\n")
print(f"Sweeping {len(CITIES)} Utah cities...\n")
...
result = supabase.table("dealerships").select("id", count="exact").eq("state", "UT").execute()
print(f"  Total UT dealerships in DB: {result.count}")
