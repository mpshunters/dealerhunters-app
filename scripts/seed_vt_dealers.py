"""
Seed Vermont motor vehicle dealers via Google Places API city sweep.
VT has no true metro — Burlington is the anchor at ~45k. The market
is spread across small cities and college towns.
Sweeps ~25 cities; deduplicates by google_place_id; only keeps state=VT.
"""
...
print("DealerHunters Vermont city-sweep seeder starting...")
...
VT_CITIES = [
    ...
    "Williston", "Shelburne", "Milton", "Colchester",
    # Central Vermont
    "Montpelier", "Barre", "Northfield", "Waterbury",
    # Rutland County
    "Rutland", "Middlebury",
    # Northeast Kingdom / North
    "St. Johnsbury", "Newport", "Morrisville", "Stowe", "St. Albans",
    # Connecticut River / Southeast
    "White River Junction", "Woodstock", "Windsor",
    # Southwest / Bennington
    "Brattleboro", "Springfield", "Bennington",
]
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} Vermont", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "VT").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing VT dealerships in DB\n")
print(f"Sweeping {len(CITIES)} Vermont cities...\n")
...
print(f"\nVermont sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "VT").execute()
print(f"  Total VT dealerships in DB: {result.count}")
