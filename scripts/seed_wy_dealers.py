"""
Seed Wyoming motor vehicle dealers via Google Places API city sweep.
WY is the least populous US state. Cheyenne and Casper anchor the market;
Gillette serves the oil-and-gas northeast. Sweeps ~22 cities;
deduplicates by google_place_id; only keeps state=WY.
"""
...
print("DealerHunters Wyoming city-sweep seeder starting...")
...
WY_CITIES = [
    # Southeast — Cheyenne metro
    "Cheyenne", "Laramie", "Wheatland", "Torrington",
    # Central — Casper metro
    "Casper", "Douglas", "Rawlins", "Lander", "Riverton",
    # Northeast — energy corridor
    "Gillette", "Sheridan", "Buffalo", "Newcastle", "Worland",
    # Northwest — tourism / ag
    "Cody", "Powell", "Thermopolis",
    # Southwest — I-80 corridor
    "Rock Springs", "Green River", "Evanston", "Kemmerer",
    # Northwest corner
    "Jackson",
]
...
def search_dealers_in_city(city: str):
    resp = requests.post(
        PLACES_URL,
        json={"textQuery": f"car dealer {city} Wyoming", "includedType": "car_dealer", "maxResultCount": 20},
...
existing = supabase.table("dealerships").select("google_place_id").eq("state", "WY").execute()
existing_place_ids = {r["google_place_id"] for r in existing.data if r.get("google_place_id")}
print(f"Found {len(existing_place_ids)} existing WY dealerships in DB\n")
print(f"Sweeping {len(CITIES)} Wyoming cities...\n")
...
print(f"\nWyoming sweep complete.\n  {total_added} added | {total_skipped} skipped | {total_errors} errors")
result = supabase.table("dealerships").select("id", count="exact").eq("state", "WY").execute()
print(f"  Total WY dealerships in DB: {result.count}")
